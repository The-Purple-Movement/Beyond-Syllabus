...
export interface AIFormat {
  ...
  structure: 'bulleted-list' | 'numbered-list' | 'step-by-step' | 'table' | 'markdown' | 'paragraph' | 'dialogue' | 'json' | 'custom';
...
export interface AIReferences {
  ...
  specificSources?: {
    name: string;
    type: 'textbook' | 'research-paper' | 'documentation' | 'course-material' | 'online-resource';
    url?: string;
    author?: string;
  }[];
}
...
export const PersonaTemplates = {
  PROFESSOR: {
    ...
    expertise: ["Deep subject knowledge", "Academic communication", "Curriculum design"],
  ...
  TUTOR: {
    ...
    expertise: ["Clear explanations", "Practice problems", "Concept reinforcement"],
    tone: "tutor" as const,
...
export const TaskTemplates = {
  SUMMARIZE: {
    ...
    objectives: ["Extract key concepts", "Identify main learning goals", "Highlight important points"],
    deliverables: ["Clear summary", "Key takeaways", "Learning objectives"]
  },
  GENERATE_QUESTIONS: {
    action: "Generate beyond syllabus questions",
    objectives: ["Create challenging questions", "Test deep understanding", "Encourage critical thinking"],
    deliverables: ["Question set", "Answer guidelines", "Difficulty levels"]
  },
  COMPARE: {
    action: "Compare different curricula or concepts",
    objectives: ["Identify similarities", "Highlight differences", "Analyze strengths and weaknesses"],
    deliverables: ["Comparison analysis", "Summary table", "Recommendations"]
  },
  EXPLAIN: {
    action: "Explain complex concepts clearly",
    objectives: ["Break down complexity", "Provide examples", "Ensure understanding"],
...
export const FormatTemplates = {
  BULLETED_LIST: {
    ...
    requirements: ["Use clear bullet points", "Organize by importance", "Keep items concise"],
    specialFormatting: { includeHeaders: true }
  },
  STEP_BY_STEP: {
    structure: "step-by-step" as const,
    requirements: ["Number each step clearly", "Provide detailed instructions", "Include examples"],
    specialFormatting: { includeHeaders: true, useCodeBlocks: true }
  },
  TABLE_FORMAT: {
    structure: "table" as const,
    requirements: ["Clear column headers", "Consistent row formatting", "Easy to read"],
    specialFormatting: { includeTables: true, includeHeaders: true }
  },
  MARKDOWN_COMPREHENSIVE: {
    structure: "markdown" as const,
    requirements: ["Use proper markdown syntax", "Include code blocks where needed", "Clear section headers"],
    specialFormatting: { 
      useCodeBlocks: true, 
      includeTables: true, 
      includeHeaders: true, 
